...
const { updateVariantStock, EcommerceApiError, MaxRetriesExceededError, computeDelay } = ecommerceApi;

const BASE = { storeId: '123456', productId: '456', variantId: '789', stock: 42, accessToken: 'test-token', skuCode: 'SKU-001' };
const STOCK_PATH = '/v1/123456/products/456/variants/stock';
...
describe('updateVariantStock — sucesso', () => {
  ...
  test('envia headers obrigatórios', async () => {
    nock('https://api.ecommerce.example.com')
      ...
      .matchHeader('user-agent', /ERPClient/)
...
describe('updateVariantStock — erros não-retentáveis', () => {
  test('404 lança EcommerceApiError imediatamente', async () => {
    nock('https://api.ecommerce.example.com')
    ...
    // EcommerceApiError is used for all non-retryable errors
  });
});
