# Documentação Geral — Middleware de Estoque SAP → EcommerceAPI
**ecommerce-webhook-middleware · v1.0**
...
Imagine que uma empresa tem dois sistemas que precisam conversar entre si:

- O **SAP** é o sistema interno que sabe exatamente quantas peças existem no estoque — cada vez que um produto entra ou sai do armazém, ele avisa.
- A **EcommerceAPI** é a plataforma da loja virtual — é ela que exibe para o cliente quanto tem disponível de cada item.

O problema é que esses dois sistemas falam em velocidades muito diferentes. O SAP pode gritar avisos a uma velocidade de **até 500 por segundo**. A EcommerceAPI, por sua vez, só consegue ouvir **500 mensagens por segundo** sem entrar em colapso — e qualquer pico acima disso resulta em erros 429 (rate limit).

Se tentássemos conectar os dois diretamente sem controle, qualquer instabilidade no consumidor causaria perda de atualizações ou banimento da API.

### O que este sistema faz?

Este middleware age como um **intermediário inteligente** entre os dois sistemas:

```
SAP ERP (fala rápido demais)
        ↓
   [ecommerce-webhook-middleware]   ← este sistema
        ↓
 EcommerceAPI (tem rate limit)
```

Ele resolve três problemas ao mesmo tempo:

| Problema | Como o middleware resolve |
|---|---|
...
| A EcommerceAPI tem um limite de chamadas | Respeita esse limite e aguarda quando necessário |
...
- ✅ **Zero risco de banimento** — o sistema nunca viola os limites da EcommerceAPI
...
| `product_id` | Identificador do produto na EcommerceAPI |
...
**Problema que resolve:** O SAP às vezes envia a mesma informação mais de uma vez (ex: "produto X tem 50 unidades" dito duas vezes em sequência). Sem proteção, isso causaria chamadas desnecessárias à EcommerceAPI.
...
### 2.3 — Controle de velocidade (respeito ao limite da EcommerceAPI)

**Limite imposto pela EcommerceAPI:** No máximo 500 chamadas acumuladas no "balde" (equivalente a 1 segundo de tráfego pleno), com reposição de 500 chamadas por segundo.

**Como o sistema se comporta:**
- Mantém um contador interno de "fichas disponíveis" (como senhas de banco).
- Cada atualização de estoque consome uma ficha antes de enviar à EcommerceAPI.
- Se não houver fichas disponíveis, o sistema aguarda pacientemente até uma ficha ser reposta.
- Após cada resposta da EcommerceAPI, o sistema sincroniza seu contador com o valor real informado pela própria API — garantindo precisão máxima.

**Regra:** Nunca é possível sobrecarregar a EcommerceAPI. Em picos, as atualizações ficam em fila mas *nenhuma é perdida*.

---

### 2.4 — Retry automático (tentativas em caso de falha)

Se a EcommerceAPI retornar um erro, o sistema tenta novamente automaticamente. O comportamento varia pelo tipo de erro:

| Tipo de erro | O que significa | O sistema faz |
|---|---|---|
| **Muitas requisições (429)** | EcommerceAPI pediu para aguardar | Espera o tempo indicado e tenta de novo |
| **Erro do servidor (5xx)** | A EcommerceAPI teve um problema interno | Aguarda e tenta de novo com espera crescente |
| **Produto não encontrado (404)** | O produto não existe na EcommerceAPI | Descarta sem retry — é um dado inválido |
...
src/worker.js
  ...
  ├── usa → src/ecommerce-api.js (para chamar a EcommerceAPI)
  └── usa → src/logger.js        (para registrar logs)

src/ecommerce-api.js
  └── chama → EcommerceAPI (HTTPS externo)
...
6. API Call:   POST https://api.ecommerce.example.com/v1/{store_id}/products/{id}/variants/stock
...
│   ├── ecommerce-api.js       ← Client HTTP para a EcommerceAPI (usado pelo Worker)
│   ├── ecommerce-client.js    ← Client estruturado alternativo (CRUD completo)
...
├── ECOMMERCE_API_GUIDE.md     ← Referência da EcommerceAPI
...
| [`src/queue.js`](./src/queue.js) | **Configuração da fila.** Define nome, opções de retry (5x, backoff exponencial com base em 2s) e política de limpeza de jobs concluídos. | [→ ver código](./src/queue.js) |
...
| [`src/rateLimiter.js`](./src/rateLimiter.js) | **Governador de velocidade.** Implementa Token Bucket: 500 tokens máx (= 1 s de burst), reposição de 500/segundo. Aceita ajuste dinâmico via header `x-rate-limit-remaining`. | [→ ver código](./src/rateLimiter.js) |
| [`src/ecommerce-api.js`](./src/ecommerce-api.js) | **Integração com EcommerceAPI.** Realiza `POST /products/:id/variants/stock`. Trata 429 e 5xx com retry+jitter. Erros 4xx são classificados como não-retriáveis e descartados. | [→ ver código](./src/ecommerce-api.js) |
| [`src/ecommerce-client.js`](./src/ecommerce-client.js) | **Client estendido.** Versão mais completa com CRUD de produtos, busca por SKU e atualização em lote (até 50 variantes por chamada via `PATCH /products/stock-price`). | [→ ver código](./src/ecommerce-client.js) |
...
| [`.env.example`](./.env.example) | **Configuração do ambiente.** Modelo com todas as variáveis necessárias: credenciais da API, Redis, rate limit, TTL de idempotência, endereço do Loki. | [→ ver arquivo](./.env.example) |
...
| [`tests/unit/`](./tests/unit/) | **Testes unitários.** Cobrem isoladamente: `rateLimiter`, `idempotency`, `ecommerce-api`, `ecommerce-client` e `worker`. Rodam sem Redis nem rede. | [→ ver pasta](./tests/unit/) |
| [`tests/integration/`](./tests/integration/) | **Testes de integração.** Testam o fluxo completo (webhook → fila → worker → API) usando mocks com `nock`. | [→ ver pasta](./tests/integration/) |
| [`tests/e2e/`](./tests/e2e/) | **Testes E2E.** Chamam a API real. Geram evidências em HTML/JSON em `reports/`. Requerem credenciais reais no `.env`. | [→ ver pasta](./tests/e2e/) |
| [`README.md`](./README.md) | **Guia técnico completo.** Instalação, configuração, endpoints da API, observabilidade e exemplos de queries LogQL. | [→ ver arquivo](./README.md) |
| [`ECOMMERCE_API_GUIDE.md`](./ECOMMERCE_API_GUIDE.md) | **Referência da EcommerceAPI.** Endpoints, autenticação, rate limit, exemplos de payload e comportamentos específicos da plataforma. | [→ ver arquivo](./ECOMMERCE_API_GUIDE.md) |
